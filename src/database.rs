...
pub fn init_db() -> Result<Connection> { //TODO: rename
...
pub fn save_sort_preference_to_db(column: &str, ascending: bool) -> Result<(), Box<dyn std::error::Error>> {
    let conn = init_db()?;
...
pub fn load_sort_preference() -> Result<Option<SortPreference>, Box<dyn std::error::Error>> {
    ...
    match result {
        Ok(value) => {
            ...
            if parts.len() == 2 { // TODO: Error handling for more than 2
...
pub fn load_holdings_sorted(
    ...
    let col = match column {
        "uid" | "coin_type" | "coin_year" | "gold_content" | "purchase_date" | "purchase_price" => column, // TODO: Should be abstracted away for easy updating
        _ => return Err(Box::new(std::io::Error::new( // Why the nesting?
    ...
    let holding_iter = stmt.query_map([], |row| {
        Ok(GoldHolding { // must this be wrapped in an okay?
